...
namespace Kospa.Data.Context
{
    public class MeetingContext
    {
        static MeetingContext()
        {
            ...
            var meeting1 = new Meeting() { Title = "Scrum", Date = DateTime.Now.AddHours(-2).AddMinutes(39) };
            meeting1.AddUsers(user1, user4, user6);
            var meeting2 = new Meeting() { Title = "Finance", Date = DateTime.Now.AddHours(5).AddMinutes(45) };
            meeting2.AddUsers(user6, user11, user8);
            var meeting3 = new Meeting() { Title = "CRM", Date = DateTime.Now.AddHours(-12).AddMinutes(25) };
            meeting3.AddUsers(user10, user1, user5, user8);
            var meeting4 = new Meeting() { Title = "Scrum", Date = DateTime.Now.AddHours(1).AddMinutes(7) };
            meeting4.AddUsers(user2, user3, user10, user9, user7);
            var meeting5 = new Meeting() { Title = "Project Startup", Date = DateTime.Now.AddHours(-7).AddMinutes(-5) };
            meeting5.AddUsers(user6, user8, user3, user11, user7);
            var meeting6 = new Meeting() { Title = "New Employment", Date = DateTime.Now.AddHours(7).AddMinutes(23) };
            meeting6.AddUsers(user6, user5, user2, user8);
            var meeting7 = new Meeting() { Title = "Finance", Date = DateTime.Now.AddHours(26).AddMinutes(16) };
            meeting7.AddUsers(user1, user2, user3, user9, user10, user4);
            var meeting8 = new Meeting() { Title = "Scrum", Date = DateTime.Now.AddHours(36).AddMinutes(1) };
